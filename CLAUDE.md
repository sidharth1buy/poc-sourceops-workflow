...
### Payments Board — urgency highlighting (2026-08-20)
`/fulfilment/payments` color-codes and sorts payments so Finance sees what needs doing first, via `paymentUrgency(mode, dueDate, open)` in `page.tsx`: **overdue** (past `dueDate`, red/`bad`), **due soon** (within `URGENT_WITHIN_DAYS` = 5, amber/`warn`), or **ESCROW mode while still pending** (amber/`warn`, regardless of any due date — funding the escrow blocks the whole order the moment it's needed, so it never waits for a countdown). Applies everywhere a `Payment` (client/supplier leg — the ones carrying a `PaymentMode`) renders: the row background (`DataTable`'s new `rowAccent` prop), a colored pill next to the due date, the top `LegSummary` cards (an "N urgent" badge overrides the plain open-count pill), and the sort order (`openFirst`'s optional `urgencyRank` — urgent-first within the open group, on both the per-leg tabs and the By-order grouped view). Customs duty and WHL fee legs don't carry a `PaymentMode`, so they're unaffected. `createOrderFromSupplierPo` seeds a CREDIT payment's `dueDate` as `createdDate + creditDays` (falls back to 30) so the countdown has something to act on from day one; `AddPaymentModal` also collects a due date now (auto-suggested +30d on CREDIT) since it never did before.
